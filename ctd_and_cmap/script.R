...
ctd_urls <- xml_text(xml_find_all(xpath = "//table//td/a", read_html(base_url))[-1])
ctd_dn <- str_subset(ctd_urls, "dn")

ctd_data <- "http://scope.soest.hawaii.edu/FTP/scope/ctd/paragon1/" %>%
  read_html() %>%
...
grid.arrange(nitr_plot, PAR_plot, chl_plot, nrow=1)
